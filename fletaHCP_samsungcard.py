...
class FletaCurl():
    ...
    def AppendnameSpaceList(self, tenname):
        ...
        cmd = 'https://%s.%s:9090/mapi/tenants/%s/namespaces?sortOrder=descending&prettyprint' % (
        tenname, self.host, tenname)
        ...
        for i in tags:
            ...
            """
            cmd = "https://%s.%s:9090/mapi/tenants/%s/namespaces/%s?sortOrder=descending&prettyprint" % (
            tenname, self.host, tenname, ns)
            cmd1 = "https://%s.%s:9090/mapi/tenants/%s/namespaces/%s/statistics?sortOrder=descending&prettyprint" % (
            tenname, self.host, tenname, ns)
    ...
    def run(self):
        ...
        self.fwrite('###***TENANTS.INPUT.ACTION.START***###')

        cmd = "https://admin.{}:8000/cluster/tenants_input.action".format(self.host)
        ...
        for ten in tenList:
            self.fwrite('###***DEFAULT.TANENTS.STATISTCS.START***###')
            cmd = 'https://admin.{HOST}:9090/mapi/tenants/{TEN}/statistics?sortOrder=descending&prettyprint'.format(
                TEN=ten, HOST=self.host)
            tenxml, recode = self.curlExeCmd(auth, cmd)
            self.fwrite(tenxml)
            self.fwrite('---TDEFAULT.TANENTS.STATISTCS.END---')
...
curl -k -i -H "Accept: application/xml" -H "Authorization: HCP YWRtaW4=:ee624467b0ed4bfe902edd3890610a9b" "https://admin.hcp.his.test.com:8000/cluster/currentPolicyStatus.action"
        ...
        cmd = "https://admin.{}:9090/mapi/nodes/statistics?prettyprint".format(self.host)
        curl_cmd = """curl -ik -H "Authorization: HCP {AUTH}" -H "Accept: application/json" "{CMD}" """.format(
            AUTH=auth, CMD=cmd)
        print(curl_cmd)
        ret = os.popen(curl_cmd).read()
        print(ret)
        ...
        nodes = jdata['nodes']['node']
        nodeIdList = list()
        for node_arg in nodes:
            nodeIdList.append(node_arg['nodeNumber'])
