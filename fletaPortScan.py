...
"""
    ...
    Changes in this release:
     - Re-use time.time() instead of time.clock(). The 2007 implementation
       worked only under Microsoft Windows. falsed on GNU/Linux.
...
class PortScan():
    ...
    def main(self,ip, port):
        pingChecking= self.verbose_ping(ip)
        print(pingChecking)
        if pingChecking:
            portChecking = self.portScan(ip, port)
        else:
            portChecking = False
        
        print('portChecking :',portChecking)
        return str(pingChecking),str(portChecking)    
